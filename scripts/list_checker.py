...
# расширенный набор буллетов (в т.ч. «» из Word/Symbol)
BULLET_CHARS = (
    "–", "-", "—",    # тире/минус
    "•", "·", "●", "∙", "◦", "‣", "",  # «кружочки/точки» (в т.ч. bullet из Wingdings/Symbol)
    "▪", "▫", "◆", "■", "□"             # квадраты/ромбы — допустимы на вложенных уровнях (валидация позже)
)

# Порог распознавания векторных буллетов
MARKER_MAX_W_MM = 8.0
MARKER_MAX_H_MM = 8.0
...
# римские/прочие — разрешим на уровнях >1 (требование «отличаться»)
...
def _rect_inter(a: fitz.Rect, b: fitz.Rect) -> fitz.Rect:
    return fitz.Rect(max(a.x0,b.x0), max(a.y0,b.y0), min(a.x1,b.x1), min(a.y1,b.y1))

def _iou(a: fitz.Rect, b: fitz.Rect) -> float:
    inter = _rect_inter(a,b)
    if inter.is_empty:
        return 0.0
    ai = inter.get_area()
    return ai / (a.get_area() + b.get_area() - ai + 1e-9)
...
def _collect_text_lines(page: fitz.Page) -> List[Line]:
    ...
    # сверху-вниз, слева-направо со снапом по Y
    y_snap = mm_to_pt(0.3)
    out.sort(key=lambda L: (round(L.bbox.y0 / y_snap)*y_snap, L.bbox.x0))
    return out

# ---------- Векторные маркеры (маленькие рисованные точки/квадраты и т.п.) ----------
def _collect_vector_markers(page: fitz.Page, exclude: List[fitz.Rect]) -> List[fitz.Rect]:
    ...
    for d in drawings:
        ...
        # пропускаем, если сильно пересекается с исключаемыми зонами (таблицы, подписи и пр.)
        skip = False
        for ex in exclude:
            if not _rect_inter(r, ex).is_empty and _iou(r, ex) > 0.30:
                skip = True; break
        if skip:
            continue

        w = r.width; h = r.height
        # маленькие объекты — кандидаты на маркеры
        if w <= MARKER_MAX_W_PT and h <= MARKER_MAX_H_PT:
            markers.append(r)
    return markers
...
def _classify_marker_and_level(
    line: Line,
    work_left: float,
    top_level_marker_kind: Optional[str],
    vector_markers: List[fitz.Rect]
) -> Optional[Item]:
    t = line.text

    # оценка уровня по левому краю относительно рабочей области
    x0 = line.bbox.x0
    dx = x0 - work_left
    approx_level = int(round(dx / INDENT_STEP_PT))
    if approx_level < 0: approx_level = 0
    # допускаем гуляние — проверим позже

    # 1) пробуем текстовые номера/маркеры
    marker_text = ""
    kind = ""
    number_kind = ""
    ...
    # 2) если текстового маркера нет — ищем рядом слева векторный «буллет»
    if not kind and vector_markers:
        # допустим небольшой горизонтальный коридор слева от текста:
        # от (work_left - 6 pt) до (x0 + 0.6 * INDENT_STEP_PT), и приличное перекрытие по Y
        x_left_min = work_left - 6.0
        x_left_max = x0 + 0.6 * INDENT_STEP_PT
        best = None
        best_dx = None
        for r in vector_markers:
            # по Y нужно разумное перекрытие (>= 40% высоты меньшего)
            ov = _y_overlap(r, line.bbox)
            min_h = max(1.0, min(r.height, line.bbox.height))
            if ov / min_h < 0.4:
                continue
            # по X — объект должен быть левее или близко к левому краю текста
            if r.x0 < x_left_min or r.x1 > x_left_max:
                continue
            dxm = max(0.0, line.bbox.x0 - r.x1)  # расстояние от маркера до начала текста
            if (best is None) or (dxm < best_dx):
                best, best_dx = r, dxm
        if best is not None:
            marker_text = "[vector]"
            kind = "bulleted"; number_kind = ""
...
def _cluster_items_to_lists(items: List[Item]) -> List[FoundList]:
    ...
    MAX_STEP_FACTOR = 2.2
    LEFT_TOL_PT = INDENT_TOL_PT + 8.0  # запас
    ...
    for it in items:
        ...
        # усреднённый кегль по двум строкам
        sizes = [sp.get("size", 0.0) for sp in it.line.spans + prev.line.spans]
        avg_fs = _median(sizes) or max(1.0, (it.line.size+prev.line.size)/2.0)
...
def check_lists(
    ...
    *,
    ...
    pages_with = set()
    n_lists = 0
    error_pages = set()  # страницы с нарушениями
    ...
    for pidx, page in enumerate(pdf_document):
        ...
        if exclude_bboxes_by_page and page_num in exclude_bboxes_by_page:
            ...
            for ln in lines:
                ...
                drop = False
                for bb in exb:
                    inter = _rect_inter(lb, bb)
                    if not inter.is_empty and (inter.get_area()/max(1.0, lb.get_area())) >= 0.30:
                        drop = True; break
                if not drop: keep.append(ln)
            lines = keep

        # соберём кандидаты векторных маркеров
        vector_markers = _collect_vector_markers(page, exb)

        top_level_marker_kind: Optional[str] = None

        # формируем кандидатов-элементов
        candidates: List[Item] = []
        for ln in lines:
            item = _classify_marker_and_level(ln, work_left, top_level_marker_kind, vector_markers)
            if item:
        ...
        for fl in found:
            ...
            for it in fl.items:
                ...
                # Ур.1: буллет только EN DASH; нумерация — digits) или rusalpha)
                if it.level == 0:
                    if it.kind == "bulleted" and it.marker_text not in (EN_DASH, "[vector]"):
                        # если это текстовый буллет — требуем именно EN DASH; если векторный — допускаем,
                        # но всё равно укажем нарушение, если хочешь — убери эту ветку.
                        if it.marker_text != EN_DASH:
                    ...
                    # уровни >1: вид должен отличаться от верхнего уровня
                ...
                # не более одного пробела после текстового маркера
                s = it.line.text
                if it.kind == "bulleted":
                    if it.marker_text == EN_DASH and not RE_ONLY_ONE_SPACE_AFTER_DASH.match(s):
                        issues.append("После «–» должен быть ровно один пробел.")
                    # для [vector] проверить точно нельзя — пропустим
                else:
                    if not RE_ONLY_ONE_SPACE_AFTER.match(s):
                        issues.append("После номера должен быть ровно один пробел.")

                # строчная буква в начале пункта
            ...
            # двоеточие перед списком и отсутствие пустых строк
            if fl.items:
            ...
            if issues:
                ...
                if annotate_pdf:
                    try:
                        ann = page.add_text_annot(
                            fitz.Point(fl.bbox.x0, fl.bbox.y0),
