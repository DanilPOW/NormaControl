...
def check_headings(doc:fitz.Document,*,start_page:int=3,annotate_pdf:bool=True)->dict:
    found: Dict[int,List[Tuple[fitz.Rect,str,int,float,int]]] = {}  # page: [(rect, txt, lvl, size, idx_line)]
    ...
    for pno, items in found.items():
        ...
        # 3) интервалы до/после (пустая строка ~ одна строка, и после ~8pt)
        for rect, txt, lvl, sz, idx in items:
            ...
            # грубая оценка мультистрочности заголовка
    ...
    user_summary = ("✅Проверка заголовков: нарушений не обнаружено" if not violations_pages
