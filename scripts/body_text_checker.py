...
def _annot(page: fitz.Page, x: float, y: float, msg: str):
    try:
        ...
        a = page.add_text_annot(fitz.Point(x,y), t); a.set_info(title=FILTER_OWN_ANNOTS_PREFIX, content=t); a.update()
    except: pass
...
def _merge_lines(raw: List[Tuple[fitz.Rect,List[dict],str,bool]]):
    ...
    for bbox,spans,txt,is_sp in raw:
        ...
        if (cur is None) or (abs(y0-cur[0].y0)>Y_MERGE_EPS):
            ...
            cur[1].extend(spans); cur[2]=(cur[2]+" "+txt).strip(); cur[3]=cur[3] and is_sp
    if cur: merged.append(cur)
    return [(b, s, t.strip(), (sp and t.strip()=="")) for b,s,t,sp in merged]
...
class Para:
    def __init__(self, page_num:int):
        self.page_num=page_num; self.lines: List[Tuple[fitz.Rect,List[dict]]]=[]
        self.all_bold=True; self.x0_first=0.0; self.y0_first=0.0
    def add(self,bbox:fitz.Rect,spans:List[dict]):
        if not self.lines:
            self.x0_first=_line_x0x1(spans)[0]; self.y0_first=bbox.y0
        self.lines.append((bbox,spans)); self.all_bold = self.all_bold and _line_all_bold(spans)
    def dom(self)->Tuple[str,float]:
        return _dom_span(self.lines[0][1]) if self.lines else ("",0.0)
    def spacing_issues(self)->List[str]:
        issues=[]
        for i in range(len(self.lines)-1):
            bi,si=self.lines[i]; bj,sj=self.lines[i+1]
...
def check_body_text(
    ...
    for pno in range(start0,total_pages):
        ...
        while i<len(vis):
            ...
            paras[-1].add(bb,sp); last_sp=False
            i+=1
        ...
        for pa in paras:
            ...
            if not _size_ok(sz): errs.append("кегль вне 12–14 pt")
            errs += pa.spacing_issues()
            if errs:
                page_issues+=1; total_issues+=1; error_pages.append(pa.page_num)
                if annotate_pdf:
                    x,y=_pin_point(pa.lines[0][1]); _annot(page,x,y," | ".join(sorted(set(errs))))
                admin_lines.append(f"[Стр. {pa.page_num}] Абзац на y≈{_fmt(pa.y0_first)}: " + " | ".join(sorted(set(errs))))
        page_stats.append((pno+1,len(paras),page_issues))

    per_page=[f"Стр. {n}: проверено абзацев {c}, нарушений {i}" for n,c,i in page_stats]
    counts = f"Проверено абзацев основного текста: {total_paras}\nВсего нарушений: {total_issues}\n" + ("\n".join(per_page) if per_page else "Страниц с текстом не найдено.")
    ...
    if lines_debug:
        admin_details += "\n\n" + "\n".join(lines_debug)
    user_summary = ("✅Проверка основного текста: нарушений не обнаружено" if total_issues==0
