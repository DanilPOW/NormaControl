...
with gr.Blocks(title="Анализатор кавычек в PDF", theme=gr.themes.Soft()) as iface:
    ...
    with gr.Accordion("ℹ️ О проверке", open=False):
        gr.Markdown("""
        - Ищем неправильные кавычки: `" ' “ ” ‘ ’ „ ‚ ‹ ›`
        - Добавляем текстовые аннотации рядом с каждым нарушением
        - Итоговый файл именуем: `<ИсходноеИмя>_Проверено_DD.MM.YYYY_в_HH:MM.pdf`
        """)
